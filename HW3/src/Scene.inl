...
void Scene::init(void){
    ...
    material["water"] -> ambient = vec4(0.1f, 0.3f, 1.0f, 1.0f);
    material["water"] -> diffuse = vec4(0.0f, 0.3f, 1.0f, 1.0f);
    material["water"] -> specular = vec4(0.9f, 0.9f, 0.9f, 1.0f);
    material["water"] -> shininess = 60.0f;
    ...
    // new model (water)
    model["stream"] = new Model;
    model["stream"] -> geometry = geometry["cube"];
    model["stream"] -> material = material["water"];
    model["puddle"] = new Model;
    model["puddle"]->geometry = geometry["cube"];
    model["puddle"]->material = material["water"];
    ...
    node["bunny"] = new Node;
    // new node
    node["stream"] = new Node;
    node["stream2"] = new Node;
    node["puddle"] = new Node;

    node["stream"] -> models.push_back(model["stream"]);
    node["stream"] -> modeltransforms.push_back( scale(vec3(0.02f, 1.0f, 0.02f)) );
    node["stream2"]->models.push_back(model["stream"]);
    node["stream2"]->modeltransforms.push_back(scale(vec3(0.02f, 1.2f, 0.02f)));
    node["puddle"] -> models.push_back(model["puddle"]);
    node["puddle"] -> modeltransforms.push_back(scale(vec3(2.0f, 0.10f, 1.0f)));

    // puddle under streams
    node["stream"]->childnodes.push_back(node["puddle"]);
    node["stream"]->childtransforms.push_back(translate(vec3(0.04f, -0.5f, 0.5f)) * rotate(-90.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
    node["stream2"]->childnodes.push_back(node["puddle"]);
    node["stream2"]->childtransforms.push_back(translate(vec3(-0.5f, -0.5425f, 0.0f)) * scale(vec3(1.6f, 1.0f, 1.0f)));
    ...
    node["table top"] -> childtransforms.push_back( translate(vec3(0.0f,-0.19f,0.0f)) * scale(vec3(0.6f, 0.6f, 0.6f))* rotate(180.0f * float(M_PI) / 180.0f, vec3(1.0f, 0.0f, 0.0f)));
    ...
    // stream under teapots
    node["teapot1"] -> childnodes.push_back( node["stream"] );
    node["teapot1"] -> childtransforms.push_back( translate(vec3(0.9f,0.04f,0.0f)) * rotate(-45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f)));
    node["teapot2"]->childnodes.push_back(node["stream2"]);
    node["teapot2"]->childtransforms.push_back(translate(vec3(0.97f, 0.17f, 0.0f))* rotate(-45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f)));
    
    node["world"] -> childnodes.push_back( node["table"] );
    node["world"] -> childtransforms.push_back(translate(vec3(0.0f, 1.54f, 0.0f)) * rotate(180.0f * float(M_PI) / 180.0f, vec3(1.0f, 0.0f, 0.0f)));
    // teapots tilted
    node["world"] -> childnodes.push_back( node["teapot1"] );
    node["world"] -> childtransforms.push_back( translate(vec3(-2.0f, 1.5f, 0.0f)) * rotate( 45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f)) );
    node["world"] -> childnodes.push_back(node["teapot1"]);
    node["world"] -> childtransforms.push_back(translate(vec3(2.0f, 1.5f, 0.0f)) * rotate(-45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f)) * rotate(180.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
    node["world"]->childnodes.push_back(node["teapot2"]);
    node["world"]->childtransforms.push_back(translate(vec3(-0.5f, 1.5f, -1.0f))* rotate(45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f)));
    node["world"]->childnodes.push_back(node["teapot2"]);
    node["world"]->childtransforms.push_back(translate(vec3(0.5f, 1.5f, 1.0f))* rotate(-45.0f * float(M_PI) / 180.0f, vec3(0.0f, 0.0f, -0.5f))* rotate(180.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
